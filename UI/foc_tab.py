...
def draw_focused_driver_telemetry(app, leader_lap, get_screen_coords, draw_track, box_geometry=(50, 160, 900, 500)):
    ...
    if pos is not None and pos != (0, 0):
        ...
        if app.raw_x is not None and app.raw_y is not None:
            ...
            draw_track(track_fx, track_fy, app.sorted_drivers,
                       leader_lap, app.db_path)

        arcade.draw_circle_filled(fx, fy, 7, color)
        arcade.draw_circle_outline(fx, fy, 9, arcade.color.WHITE, 2)
        arcade.draw_text(f"{abbr} [{rank_text}]", fx + 18, fy,
        ...
        arcade.draw_rect_filled(arcade.XYWH(
            center_x, center_y, box_w, box_h), arcade.color.BLACK)
        arcade.draw_rect_outline(arcade.XYWH(
            center_x, center_y, box_w, box_h), arcade.color.DARK_GRAY, border_width=2)

        legend_y = box_y + box_h + 15
        arcade.draw_text("LIVE TELEMETRY PANEL:", box_x,
                         legend_y, arcade.color.WHITE, 12, bold=True)

        arcade.draw_text("■ SPEED",    box_x + 230, legend_y,
                         color,                               11, bold=True)
        arcade.draw_text("■ RPM",      box_x + 340, legend_y,
                         arcade.color.LIGHT_GOLDENROD_YELLOW, 11, bold=True)
        arcade.draw_text("■ THROTTLE", box_x + 430, legend_y,
                         arcade.color.GREEN,                  11, bold=True)
        arcade.draw_text("■ BRAKE",    box_x + 550, legend_y,
                         arcade.color.RED,                    11, bold=True)

        max_t = 100.0 if (hist_throttle is not None and len(
            hist_throttle) > 0 and max(hist_throttle) > 1.1) else 1.0
        max_b = 100.0 if (hist_brake is not None and len(
            hist_brake) > 0 and max(hist_brake) > 1.1) else 1.0
