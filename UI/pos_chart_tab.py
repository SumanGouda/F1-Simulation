...
def draw_position_chart(db_path, total_laps, total_drivers, origin_x, origin_y, plot_w, plot_h,
    ...
    grid_to_abbr = {positions[0]: abbr for abbr, positions in driver_lap_positions.items() if positions}

    # --- Background ---
    arcade.draw_rect_filled(
        arcade.rect.XYWH(origin_x + plot_w / 2, origin_y + plot_h / 2, plot_w, plot_h), (10, 10, 10)
    )
    ...
    for pos in range(1, total_drivers + 1):
        ...
        driver_at_pos       = grid_to_abbr.get(pos, str(pos))
            ...
            if is_retired:
                if retired_icon:
                    arcade.draw_texture_rect(
                        retired_icon, arcade.rect.XYWH(last_x + icon_size / 2, last_y, icon_size, icon_size)
                    )
            else:
                arcade.draw_text(
                    abbr, last_x + icon_size + 5, last_y, color, 12, bold=True, anchor_x="left", anchor_y="center"
                )
                if finished_icon:
                    arcade.draw_texture_rect(
                        finished_icon, arcade.rect.XYWH(last_x + icon_size / 2, last_y, icon_size, icon_size)
                    )
